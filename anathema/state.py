from __future__ import annotations
from typing import TYPE_CHECKING, List, Any

from morphism import Size  # type: ignore

import logging
from abc import ABCMeta

from anathema import prepare
from anathema.views import FirstResponderContainerView

if TYPE_CHECKING:
    from anathema.client import Client
    from anathema.views.view import View
    from tcod.event import KeyboardEvent


logger = logging.getLogger(__name__)


def add_input_handler(state: State, handler: View) -> None:
    if not getattr(handler, "handle_input"):
        raise ValueError("Invalid handler")
    state.input_handlers.append(handler)


def remove_input_handler(state: State, handler: View) -> None:
    state.input_handlers.remove(handler)


class State(metaclass=ABCMeta):
    """Prototype class for States.

    All states should inherit from this. No direct instances of this class
    should be created. Update must be overloaded in the child classes.

    Methods:
        - startup           Called when added to the state stack.
        - resume            Called each time state is updated for the first time.
        - update            Called each frame while state is active.
        - process_event     Called when there is a new input event.
        - pause             Called when state is no longer active.
        - shutdown          Called before state is destroyed.
    """

    def __init__(self, client: Client, views: List[View]) -> None:
        self.client = client
        if not isinstance(views, list):
            views = [views]  # type: ignore
        self.view = FirstResponderContainerView(subviews=views, state=self)
        add_input_handler(self, self.view)

        self.start_time: float = 0.0
        self.current_time: float = 0.0
        self._input_handlers: List[View] = []

    @property
    def name(self) -> str:
        return self.__class__.__name__

    @property
    def input_handlers(self) -> List[View]:
        return self._input_handlers

    def handle_input(self, event: KeyboardEvent) -> None:
        for handler in self._input_handlers:
            handler.handle_input(event)

    def on_enter(self, *args: List[Any]) -> None:
        """Lifecycle hook run when the State becomes active."""
        pass

    def on_leave(self, *args: List[Any]) -> None:
        """Lifecycle hook run when the State is resigned as active."""
        pass

    def become_active(self) -> None:
        """Lifecycle hook for initial setup of the state on becoming active."""
        pass

    def resign_active(self) -> None:
        """Lifecycle hook for cleanup of the state on resigning as active."""
        pass

    def pre_update(self) -> None:
        """Timing hook that runs just before the screen update step."""
        pass

    def on_update(self, is_active: bool = False) -> bool:
        """Timing hook that runs during the update step.

        In general, do not override this method.
        """
        self.pre_update()
        self.view.frame = self.view.frame.with_size(
            Size(prepare.CONSOLE_WIDTH, prepare.CONSOLE_HEIGHT)
        )
        self.view.perform_layout()
        self.view.perform_draw()
        self.post_update()
        return True

    def post_update(self) -> None:
        """Timing hook that runs just after the screen update step."""
        pass
